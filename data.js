...
app.get('/users/get',(req, res)=>{
  ...
  const SELECT_USER_ID_QUERY = `SELECT * FROM users WHERE name = '${username}'AND password = '${password}'`;
  connection.query(SELECT_USER_ID_QUERY, (err, results)=>{
    if(err){
      return res.send(err)
    }
...
app.get('/users/add', (req, res) => {
  ...
  const INSERT_USERS_QUERY = `INSERT INTO users (id, name, email, password) VALUES(0,'${name}','${email}','${password}')`;
